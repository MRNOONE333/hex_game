...
class UI:
    ...
    def get_normalized_visits(self, visits: list):
        if not visits or all(visit == 0 for visit in visits):
            return [0] * len(visits)

        normalized_visits = [node_visits - min(visits) for node_visits in visits]
        max_normalized_visits = max(normalized_visits)
        # Avoid division by zero
        return [int(node_visits / max_normalized_visits * 200) if max_normalized_visits != 0 else 0
                for node_visits in normalized_visits]
